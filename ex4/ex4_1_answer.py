...
for k, (train_itr, test_itr) in enumerate(kfold):
    ...
    svm_turned = svm.SVC(kernel='rbf', gamma=gs.best_params_['gamma'])
    svm_turned.fit(X[train_itr], y[train_itr])
    acc_trn = svm_turned.score(X[train_itr],y[train_itr])
    acc_tst = svm_turned.score(X[test_itr],y[test_itr])
    acc_trn_list.append(acc_trn)
